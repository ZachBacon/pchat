...
winamp(char *word[], char *word_eol[], void *userdata)
{
	...
	if (hwndWinamp)
	{
		if (!stricmp("PAUSE", word[2]))
		{
			if (SendMessage(hwndWinamp,WM_USER, 0, 104))
			{
				...
				if (SendMessage(hwndWinamp, WM_USER, 0, 104) == PLAYING)
					pchat_printf(ph, "Winamp: playing");
				else
					pchat_printf(ph, "Winamp: paused");
			}
		}
		else if (!stricmp("STOP", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40047, 0);
			pchat_printf(ph, "Winamp: stopped");
		}
		...
		else if (!stricmp("NEXT", word[2]))
		{
			...
			pchat_printf(ph, "Winamp: next playlist entry");
		}
		else if (!stricmp("PREV", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40044, 0);
			pchat_printf(ph, "Winamp: previous playlist entry");
		}
		else if (!stricmp("START", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40154, 0);
			pchat_printf(ph, "Winamp: playlist start");
		}
		else if (!word_eol[2][0])
		{
			...
			if (!current_play)
			{
				pchat_print (ph, "Winamp: Error getting song information.");
			...
			if (strchr(current_play, '-'))
			{
				...
				*++p = '\0';
				...
				if (*p != '\0')
					pchat_commandf (ph, "me is now playing: %s", p);
				else
					pchat_print (ph, "Winamp: No song information found.");
				g_free (current_play);
			}
			else
				pchat_print(ph, "Winamp: Nothing being played.");
		}
		else
			pchat_printf(ph, "Usage: /WINAMP [PAUSE|PLAY|STOP|NEXT|PREV|START]\n");
	}
	else
	{
		pchat_print(ph, "Winamp not found.\n");
	}
	return PCHAT_EAT_ALL;
}
					  ...
					  char *arg)
{
	...
	*plugin_name = "Winamp";
	*plugin_desc = "Winamp plugin for PChat";
	*plugin_version = "0.6";

	pchat_hook_command (ph, "WINAMP", PCHAT_PRI_NORM, winamp, "Usage: /WINAMP [PAUSE|PLAY|STOP|NEXT|PREV|START] - control Winamp or show what's currently playing", 0);
   	pchat_command (ph, "MENU -ishare\\music.png ADD \"Window/Display Current Song (Winamp)\" \"WINAMP\"");
	pchat_print (ph, "Winamp plugin loaded\n");
...
{
	pchat_command (ph, "MENU DEL \"Window/Display Current Song (Winamp)\"");
	pchat_print (ph, "Winamp plugin unloaded\n");
	return 1;
}
