...
with open(sys.argv[1]) as f:
    output = []
    eat_until_endif = 0
    # This is very specific to pchat-plugin.h, it is not a cpp
    for line in f:
        if line.startswith('#define'):
            continue
        elif line.endswith('PCHAT_PLUGIN_H\n'):
            continue
        elif 'time.h' in line:
            ...
            eat_until_endif -= 1
        elif eat_until_endif and '_pchat_context' not in line:
            continue
        else:
            output.append(line)
...
extern "Python" int _on_print_attrs_hook(char **, pchat_event_attrs *, void *);
extern "Python" int _on_server_hook(char **, char **, void *);
extern "Python" int _on_server_attrs_hook(char **, char **, pchat_event_attrs *, void *);
...
builder.set_source('_pchat_embedded', '''
                        ...
                        char **version_out, char *arg)
{
    ...
    ph = plugin_handle;
    return _on_plugin_init(name_out, description_out, version_out, arg, PCHATLIBDIR);
}
