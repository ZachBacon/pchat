...
VERSION = b'2.0'  # Sync with pchat.__version__
...
def redirected_stdout():
    ...
    yield
    sys.stdout = pchat_stdout
    sys.stderr = pchat_stdout


if os.getenv('PCHAT_LOG_PYTHON'):
    def log(*args):
...
class Stdout:
    ...
    def write(self, string):
        ...
        if idx != -1:
            ...
            lib.pchat_print(lib.ph, bytes(self.buffer))
...
class Plugin:
    ...
    def loadfile(self, filename):
        try:
            ...
            try:
                ...
                lib.pchat_print(lib.ph, b'Failed to load module: __module_name__ must be set')

                return False

            self.version = self.globals.get('__module_version__', '')
            self.description = self.globals.get('__module_description__', '')
            self.ph = lib.pchat_plugingui_add(lib.ph, filename.encode(), self.name.encode(),
                                                self.description.encode(), self.version.encode(), ffi.NULL)

        except Exception as e:
            lib.pchat_print(lib.ph, 'Failed to load module: {}'.format(e).encode())
            traceback.print_exc()
            return False
    ...
    def __del__(self):
        ...
        if self.ph is not None:
            lib.pchat_plugingui_remove(lib.ph, self.ph)
...
def _on_timer_hook(userdata):
    ...
    try:
        # Avoid calling pchat_unhook twice if unnecessary
        hook.is_unload = True
...
def _on_say_command(word, word_eol, userdata):
    channel = ffi.string(lib.pchat_get_info(lib.ph, b'channel'))
    if channel == b'>>python<<':
        python = ffi.string(word_eol[1])
        lib.pchat_print(lib.ph, b'>>> ' + python)
...
def load_filename(filename):
    ...
    if not os.path.isabs(filename):
        configdir = __decode(ffi.string(lib.pchat_get_info(lib.ph, b'configdir')))
...
def list_plugins():
    if not plugins:
        lib.pchat_print(lib.ph, b'No python modules loaded')
        return
    ...
    for row in tbl:
        lib.pchat_print(lib.ph, b' '.join(item.ljust(column_sizes[i])
                                            for i, item in enumerate(row)))
    lib.pchat_print(lib.ph, b'')
...
def exec_in_interp(python):
    ...
    if local_interp is None:
        local_interp = Plugin()
        local_interp.locals = {}
        local_interp.globals['pchat'] = pchat

    code = compile_line(python)
    try:
        ret = eval(code, local_interp.globals, local_interp.locals)
        if ret is not None:
            lib.pchat_print(lib.ph, '{}'.format(ret).encode())

    except Exception as e:
        traceback.print_exc(file=pchat_stdout)
...
def _on_py_command(word, word_eol, userdata):
    ...
    if subcmd == 'exec':
        ...
        if not unload_name(name):
            lib.pchat_print(lib.ph, b'Can\'t find a python plugin with that name')
        ...
        lib.pchat_command(lib.ph, b'QUERY >>python<<')

    elif subcmd == 'list':
        list_plugins()

    elif subcmd == 'about':
        lib.pchat_print(lib.ph, b'PChat Python interface version ' + VERSION)

    else:
        lib.pchat_command(lib.ph, b'HELP PY')
...
def _on_plugin_init(plugin_name, plugin_desc, plugin_version, arg, libdir):
    ...
    try:
        ...
        lib.pchat_print(lib.ph, b'Failed to import module: ' + repr(e).encode())
    ...
    lib.pchat_hook_command(lib.ph, b'', 0, lib._on_say_command, ffi.NULL, ffi.NULL)
    lib.pchat_hook_command(lib.ph, b'LOAD', 0, lib._on_load_command, ffi.NULL, ffi.NULL)
    lib.pchat_hook_command(lib.ph, b'UNLOAD', 0, lib._on_unload_command, ffi.NULL, ffi.NULL)
    lib.pchat_hook_command(lib.ph, b'RELOAD', 0, lib._on_reload_command, ffi.NULL, ffi.NULL)
    lib.pchat_hook_command(lib.ph, b'PY', 0, lib._on_py_command, b'''Usage: /PY LOAD   <filename>
    ...
    lib.pchat_print(lib.ph, b'Python interface loaded')
    autoload()
    return 1
