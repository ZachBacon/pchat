...
from _pchat_embedded import ffi, lib
...
__doc__ = 'PChat Scripting Interface'
__version__ = (2, 0)
__license__ = 'GPL-2.0+'
...
# ------------ API ------------
def prnt(string):
    lib.pchat_print(lib.ph, string.encode())
...
def emit_print(event_name, *args, **kwargs):
    ...
    if time == 0:
        return lib.pchat_emit_print(lib.ph, event_name.encode(), *cargs)

    attrs = lib.pchat_event_attrs_create(lib.ph)
    attrs.server_time_utc = time
    ret = lib.pchat_emit_print_attrs(lib.ph, attrs, event_name.encode(), *cargs)
    lib.pchat_event_attrs_free(lib.ph, attrs)
    return ret


# TODO: this shadows itself. command should be changed to cmd
def command(command):
    lib.pchat_command(lib.ph, command.encode())


def nickcmp(string1, string2):
    return lib.pchat_nickcmp(lib.ph, string1.encode(), string2.encode())

def strip(text, length=-1, flags=3):
    stripped = lib.pchat_strip(lib.ph, text.encode(), length, flags)
    ret = __decode(ffi.string(stripped))
    lib.pchat_free(lib.ph, stripped)
    return ret


def get_info(name):
    ret = lib.pchat_get_info(lib.ph, name.encode())
    if ret == ffi.NULL:
        return None
...
def get_prefs(name):
    ...
    int_out = ffi.new('int*')
    _type = lib.pchat_get_prefs(lib.ph, name.encode(), string_out, int_out)
    if _type == 0:
        return None
...
def __get_fields(name):
    return __FIELD_CACHE.setdefault(name, __cstrarray_to_list(lib.pchat_list_fields(lib.ph, name)))
...
def get_list(name):
    ...
    list_ = lib.pchat_list_get(lib.ph, name)
    if list_ == ffi.NULL:
        return None
    ...
    def string_getter(field):
        string = lib.pchat_list_str(lib.ph, list_, field)
        if string != ffi.NULL:
    ...
    def ptr_getter(field):
        if field == b'context':
            ptr = lib.pchat_list_str(lib.ph, list_, field)
            ctx = ffi.cast('pchat_context*', ptr)
            return Context(ctx)
    ...
    getters = {
        ord('s'): string_getter,
        ord('i'): lambda field: lib.pchat_list_int(lib.ph, list_, field),
        ord('t'): lambda field: lib.pchat_list_time(lib.ph, list_, field),
        ord('p'): ptr_getter,
    }

    while lib.pchat_list_next(lib.ph, list_) == 1:
    ...
    lib.pchat_list_free(lib.ph, list_)
    return ret
...
def hook_command(command, callback, userdata=None, priority=PRI_NORM, help=None):
    ...
    handle = lib.pchat_hook_command(lib.ph, command.encode(), priority, lib._on_command_hook,
                                      help.encode() if help is not None else ffi.NULL, hook.handle)

    hook.pchat_hook = handle
    return id(hook)
...
def hook_print(name, callback, userdata=None, priority=PRI_NORM):
    plugin = __get_current_plugin()
    hook = plugin.add_hook(callback, userdata)
    handle = lib.pchat_hook_print(lib.ph, name.encode(), priority, lib._on_print_hook, hook.handle)
    hook.pchat_hook = handle
    return id(hook)
...
def hook_print_attrs(name, callback, userdata=None, priority=PRI_NORM):
    plugin = __get_current_plugin()
    hook = plugin.add_hook(callback, userdata)
    handle = lib.pchat_hook_print_attrs(lib.ph, name.encode(), priority, lib._on_print_attrs_hook, hook.handle)
    hook.pchat_hook = handle
    return id(hook)
...
def hook_server(name, callback, userdata=None, priority=PRI_NORM):
    plugin = __get_current_plugin()
    hook = plugin.add_hook(callback, userdata)
    handle = lib.pchat_hook_server(lib.ph, name.encode(), priority, lib._on_server_hook, hook.handle)
    hook.pchat_hook = handle
    return id(hook)
...
def hook_server_attrs(name, callback, userdata=None, priority=PRI_NORM):
    plugin = __get_current_plugin()
    hook = plugin.add_hook(callback, userdata)
    handle = lib.pchat_hook_server_attrs(lib.ph, name.encode(), priority, lib._on_server_attrs_hook, hook.handle)
    hook.pchat_hook = handle
    return id(hook)
...
def hook_timer(timeout, callback, userdata=None):
    plugin = __get_current_plugin()
    hook = plugin.add_hook(callback, userdata)
    handle = lib.pchat_hook_timer(lib.ph, timeout, lib._on_timer_hook, hook.handle)
    hook.pchat_hook = handle
    return id(hook)
...
def set_pluginpref(name, value):
    if isinstance(value, str):
        return bool(lib.pchat_pluginpref_set_str(lib.ph, name.encode(), value.encode()))

    if isinstance(value, int):
        return bool(lib.pchat_pluginpref_set_int(lib.ph, name.encode(), value))
...
def get_pluginpref(name):
    ...
    if lib.pchat_pluginpref_get_str(lib.ph, name, string_out) != 1:
        return None
    ...
    number = lib.pchat_pluginpref_get_int(lib.ph, name)
...
def del_pluginpref(name):
    return bool(lib.pchat_pluginpref_delete(lib.ph, name.encode()))


def list_pluginpref():
    prefs_str = ffi.new('char[4096]')
    if lib.pchat_pluginpref_list(lib.ph, prefs_str) == 1:
...
class Context:
    ...
    def __change_context(self):
        old_ctx = lib.pchat_get_context(lib.ph)
        if not self.set():
            # XXX: Behavior change, previously used wrong context
            lib.pchat_print(lib.ph, b'Context object refers to closed context, ignoring call')
            return

        yield
        lib.pchat_set_context(lib.ph, old_ctx)
    def set(self):
        # XXX: API addition, C plugin silently ignored failure
        return bool(lib.pchat_set_context(lib.ph, self._ctx))
...
def get_context():
    ctx = lib.pchat_get_context(lib.ph)
    return Context(ctx)
...
def find_context(server=None, channel=None):
    ...
    ctx = lib.pchat_find_context(lib.ph, server, channel)
    if ctx == ffi.NULL:
        return None
