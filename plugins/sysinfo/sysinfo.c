...
get_client (void)
{
	return g_strdup_printf ("PChat %s", pchat_get_info(ph, "version"));
}
...
print_summary (gboolean announce)
{
	...
	pchat_commandf (ph, "%s %s", announce ? "SAY" : "ECHO", output);
...
print_info (char *info, gboolean announce)
{
	...
	for (i = 0; hwinfos[i].name != NULL; i++)
	{
		if (!g_ascii_strcasecmp (info, hwinfos[i].name))
		{
			...
			if (str)
			{
				pchat_commandf (ph, "%s \002%s\002: %s", announce ? "SAY" : "ECHO",
									hwinfos[i].title, str);
				g_free (str);
			}
			else
				pchat_print (ph, _("Sysinfo: Failed to get info. Either not supported or error."));
			return;
		}
	}

	pchat_print (ph, _("Sysinfo: No info by that name\n"));
}

static gboolean
sysinfo_get_bool_pref (const char *pref, gboolean def)
{
	int value = pchat_pluginpref_get_int (ph, pref);
...
sysinfo_set_pref_real (const char *pref, char *value, gboolean def)
{
	if (value && value[0])
	{
		...
		pchat_pluginpref_set_int (ph, pref, i != 0);
		pchat_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref, i != 0);
	}
	else
	{
		pchat_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref,
...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		pchat_print (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
	...
	pchat_print (ph, _("Sysinfo: Invalid variable name\n"));
}
...
sysinfo_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	channel_type = pchat_list_int (ph, NULL, "type");
...
pchat_plugin_init (pchat_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = version;
	...
	pchat_hook_command (ph, "SYSINFO", PCHAT_PRI_NORM, sysinfo_cb, sysinfo_help, NULL);

	pchat_command (ph, "MENU ADD \"Window/Send System Info\" \"SYSINFO\"");
	pchat_printf (ph, _("%s plugin loaded\n"), name);
	return 1;
}
...
{
	pchat_command (ph, "MENU DEL \"Window/Display System Info\"");
	pchat_printf (ph, _("%s plugin unloaded\n"), name);
	return 1;
}
