$SIG{__WARN__} = sub {
	...
	if( $message =~ /^(?:Gtk|GLib|Gdk)(?:-\w+)?-$message_levels/i ) {
		...
		if( defined &PChat::Internal::print ) {
			PChat::print( $message );
		} else {
			warn $message;
		}
...
sub get_context;
sub PChat::Internal::context_info;
sub PChat::Internal::print;
...
our %EXPORT_TAGS = (
	constants => [
		...
		qw(EAT_NONE EAT_PCHAT EAT_XCHAT EAT_PLUGIN EAT_ALL), # callback return values
...
sub register {
	my ($package, $calling_package) = PChat::Embed::find_pkg();
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $filename = $pkg_info->{filename};
	my ($name, $version, $description, $callback) = @_;
	
	if( defined $pkg_info->{gui_entry} ) {
		PChat::print( "PChat::register called more than once in "
			. $pkg_info->{filename} );
		return ();
	}
	...
	if( $callback ) {
		$callback = PChat::Embed::fix_callback(
	...
	$pkg_info->{gui_entry} =
		PChat::Internal::register( $name, $version, $description, $filename );
...
sub hook_server {
	...
	my ($priority, $data) = ( PChat::PRI_NORM, undef );
	...
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $hook = PChat::Internal::hook_server(
...
sub hook_command {
	...
	my ($priority, $help_text, $data) = ( PChat::PRI_NORM, undef, undef );
	_process_hook_options(
		$options,
	...
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $hook = PChat::Internal::hook_command(
...
sub hook_print {
	...
	my ($priority, $run_after, $filter, $data) = ( PChat::PRI_NORM, 0, 0, undef );
	_process_hook_options(
		$options,
	...
	if( $run_after and $filter ) {
		Carp::carp( "PChat::hook_print's run_after_event and filter options are mutually exclusive, you can only use of them at a time per hook" );
		return;
	}
	...
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $hook = PChat::Internal::hook_print(
...
sub hook_timer {
	...
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $hook = PChat::Internal::hook_timer( $timeout, $callback, $data, $package );
	push @{$pkg_info->{hooks}}, $hook if defined $hook;
	return $hook;
}
...
sub hook_fd {
	...
	my ($flags, $data) = (PChat::FD_READ, undef);
	_process_hook_options(
		$options,
		[qw(flags data)],
		[\($flags, $data)],
	...
	my $pkg_info = PChat::Embed::pkg_info( $package );
	my $hook = PChat::Internal::hook_fd(
		$fileno, $cb, $flags, {
...
sub unhook {
	...
	if( defined( $hook )
		...
		return PChat::Internal::unhook( $hook );
	}
	return ();
}
...
sub _do_for_each {
	...
	my $num_done = 0;
	my $old_ctx = PChat::get_context();
	for my $server ( @$servers ) {
		for my $channel ( @$channels ) {
			if( PChat::set_context( $channel, $server ) ) {
				$cb->();
				$num_done++
			}
		}
	}
	PChat::set_context( $old_ctx );
	return $num_done;
}
...
sub printf {
	my $format = shift;
	PChat::print( sprintf( $format, @_ ) );
}

# make PChat::prnt() and PChat::prntf() as aliases for PChat::print() and 
# PChat::printf(), mainly useful when these functions are exported
sub prnt {
	goto &PChat::print;
}
...
sub command {
	...
	return _do_for_each(
		sub { PChat::Internal::command( $_ ) foreach @commands },
		@_
	);
}
...
sub commandf {
	my $format = shift;
	PChat::command( sprintf( $format, @_ ) );
}
...
sub plugin_pref_set {
	...
	return PChat::Internal::plugin_pref_set($setting, $value);
}

sub plugin_pref_get {
	my $setting = shift // return 0;

	return PChat::Internal::plugin_pref_get($setting);
}

sub plugin_pref_delete {
	my $setting = shift // return 0;

	return PChat::Internal::plugin_pref_delete($setting);
}

sub plugin_pref_list {
	my %list = PChat::Internal::plugin_pref_list();
	return \%list;
}
...
sub set_context {
	...
	if( @_ == 2 ) {
		my ($channel, $server) = @_;
		$context = PChat::find_context( $channel, $server );
	} elsif( @_ == 1 ) {
		if( defined $_[0] && $_[0] =~ /^\d+$/ ) {
			$context = $_[0];
		} else {
			$context = PChat::find_context( $_[0] );
		}
	} elsif( @_ == 0 ) {
		$context = PChat::find_context();
	}
	return $context ? PChat::Internal::set_context( $context ) : 0;
}
...
sub get_info {
	...
	if( defined( $id ) ) {
		if( grep { $id eq $_ } qw(state_cursor id) ) {
			$info = PChat::get_prefs( $id );
		} else {
			$info = PChat::Internal::get_info( $id );
		}
	}
	return $info;
}
...
sub user_info {
	my $nick = PChat::strip_code(shift @_ || PChat::get_info( "nick" ));
	my $user;
	for (PChat::get_list( "users" ) ) {
		if ( PChat::nickcmp( $_->{nick}, $nick ) == 0 ) {
			$user = $_;
			last;
		}
	}
...
sub context_info {
	my $ctx = shift @_ || PChat::get_context;
	my $old_ctx = PChat::get_context;
	my @fields = (
	...
	if( PChat::set_context( $ctx ) ) {
		my %info;
		for my $field ( @fields ) {
			$info{$field} = PChat::get_info( $field );
		}
		
		my $ctx_info = PChat::Internal::context_info;
...
sub get_list {
	...
	if( $_[0] eq 'networks' ) {
		return PChat::List::Network->get();
	} else {
		return PChat::Internal::get_list( $_[0] );
	}
}
