...
#   gui_entry
#     This is pchat_plugin pointer that is used to remove the script from
...
sub unload {
	...
	if( $pkg_info ) {	
		...
		return PChat::EAT_ALL;
	} else {
		PChat::print( qq{"$file" is not loaded.\n} );
		return PChat::EAT_NONE;
	}
}
...
sub reload_all {
	my @dirs = PChat::get_info( "configdir" );
...
sub evaluate {
	...
	PChat::print $@ if $@; #print warnings

	local $Data::Dumper::Sortkeys = 1;
	local $Data::Dumper::Terse    = 1;

	if (@results > 1) {
		PChat::print Dumper \@results;
	}
	elsif (ref $results[0] || !$results[0]) {
		PChat::print Dumper $results[0];
	}
	else {
		PChat::print $results[0];
	}

	return PChat::EAT_PCHAT;
};
...
sub find_external_pkg {
	...
	while( my @frame = caller( $level ) ) {
		return @frame if $frame[0] !~ /(?:^IRC$|^PChat)/;
		$level++;
	}
	return;
}
...
sub find_pkg {
	...
	while( my ($package, $file, $line) = caller( $level ) ) {
		return $package if $package =~ /^PChat::Script::/;
		$level++;
	}
