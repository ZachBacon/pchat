...
tray_menu_cb (GtkWidget *widget, guint button, guint time, gpointer userdata)
{
	...
	blink_item (&prefs.pchat_input_tray_chans, submenu, _("Channel Message"));
	blink_item (&prefs.pchat_input_tray_priv, submenu, _("Private Message"));
	blink_item (&prefs.pchat_input_tray_hilight, submenu, _("Highlighted Message"));
...
tray_priv (char *from, char *text)
{
	...
	if (prefs.pchat_input_balloon_priv)
...
/*	if (tray_status == TS_FILEOFFER)
	...
	if (prefs.pchat_input_balloon_priv && (!prefs.pchat_away_omit_alerts || tray_find_away_status () != 1))
