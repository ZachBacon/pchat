...
notification_backend_show (const char *title, const char *text)
{
    ...
    g_variant_builder_add (&params, "s", "pchat"); /* App name */
    g_variant_builder_add (&params, "u", 0); /* ID, 0 means don't replace */
    g_variant_builder_add (&params, "s", "io.github.PChat"); /* App icon */
    ...
    g_variant_builder_add (&params, "v", g_variant_new_string ("io.github.PChat"));
