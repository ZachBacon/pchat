...
fe_userlist_numbers (session *sess)
{
	...
	if (sess == current_tab || !sess->gui->is_tab)
	{
		...
		if (sess->type == SESS_CHANNEL && prefs.pchat_gui_win_ucount)
			fe_set_title (sess);
	}
}
...
fe_userlist_rehash (session *sess, struct User *user)
{
	...
	if (prefs.pchat_away_track && user->away)
		nick_color = COL_AWAY;
...
userlist_add_columns (GtkTreeView * treeview)
{
	...
	if (prefs.pchat_gui_ulist_show_hosts)
	{
		/* hostname column */
		renderer = gtk_cell_renderer_text_new ();
		if (prefs.pchat_gui_compact)
...
userlist_click_cb (GtkWidget *widget, GdkEventButton *event, gpointer userdata)
{
	...
	if (!(event->state & STATE_CTRL) &&
		event->type == GDK_2BUTTON_PRESS && prefs.pchat_gui_ulist_doubleclick[0])
	{
		nicks = userlist_selection_list (widget, &i);
		if (nicks)
		{
			nick_command_parse (current_sess, prefs.pchat_gui_ulist_doubleclick, nicks[0],
									  nicks[0]);
			while (i)
			{
				i--;
