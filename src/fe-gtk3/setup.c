...
#ifdef _WIN32
	{ST_MENU,   N_("Language:"), P_OFFINTNL(pchat_gui_lang), 0, langsmenu, 0},
	{ST_EFONT,  N_("Main font:"), P_OFFSETNL(pchat_text_font_main), 0, 0, sizeof prefs.pchat_text_font_main},
#else
	{ST_EFONT,  N_("Font:"), P_OFFSETNL(pchat_text_font), 0, 0, sizeof prefs.pchat_text_font},
#endif

	{ST_HEADER,	N_("Text Box"),0,0,0},
	{ST_TOGGLE, N_("Colored nick names"), P_OFFINTNL(pchat_text_color_nicks), N_("Give each person on IRC a different color"),0,0},
	{ST_TOGGLR, N_("Indent nick names"), P_OFFINTNL(pchat_text_indent), N_("Make nick names right-justified"),0,0},
	{ST_TOGGLE, N_ ("Show marker line"), P_OFFINTNL (pchat_text_show_marker), N_ ("Insert a red line after the last read text."), 0, 0},
	{ST_EFILE, N_ ("Background image:"), P_OFFSETNL (pchat_text_background), 0, 0, sizeof prefs.pchat_text_background},

	{ST_HEADER, N_("Transparency Settings"), 0,0,0},
	{ST_HSCALE, N_("Window Opacity:"), P_OFFINTNL(pchat_gui_transparency),0,0,0},

	{ST_HEADER,	N_("Time Stamps"),0,0,0},
	{ST_TOGGLE, N_("Enable time stamps"), P_OFFINTNL(pchat_stamp_text),0,0,1},
	{ST_ENTRY,  N_("Time stamp format:"), P_OFFSETNL(pchat_stamp_text_format),
#ifdef _WIN32
					N_("See the strftime MSDN article for details."),0,sizeof prefs.pchat_stamp_text_format},
#else
					N_("See the strftime manpage for details."),0,sizeof prefs.pchat_stamp_text_format},
#endif
...
static const setting inputbox_settings[] =
{
	...
	{ST_TOGGLE, N_("Use the Text box font and colors"), P_OFFINTNL(pchat_gui_input_style),0,0,0},
	{ST_TOGGLE, N_("Render colors and attributes"), P_OFFINTNL (pchat_gui_input_attr),0,0,0},
	{ST_TOGGLE, N_("Show nick box"), P_OFFINTNL(pchat_gui_input_nick),0,0,1},
	{ST_TOGGLE, N_("Show user mode icon in nick box"), P_OFFINTNL(pchat_gui_input_icon),0,0,0},
	{ST_TOGGLE, N_("Spell checking"), P_OFFINTNL(pchat_gui_input_spell),0,0,1},
	{ST_ENTRY,	N_("Dictionaries to use:"), P_OFFSETNL(pchat_text_spell_langs),0,0,sizeof prefs.pchat_text_spell_langs},
#ifdef _WIN32
	...
	{ST_ENTRY,	N_("Nick completion suffix:"), P_OFFSETNL(pchat_completion_suffix),0,0,sizeof prefs.pchat_completion_suffix},
...
static const setting userlist_settings[] =
{
	...
	{ST_TOGGLE, N_("Show hostnames in user list"), P_OFFINTNL(pchat_gui_ulist_show_hosts), 0, 0, 0},
	{ST_TOGGLE, N_("Use the Text box font and colors"), P_OFFINTNL(pchat_gui_ulist_style),0,0,0},
	{ST_TOGGLE, N_("Show icons for user modes"), P_OFFINTNL(pchat_gui_ulist_icons), N_("Use graphical icons instead of text symbols in the user list."), 0, 0},
	{ST_TOGGLE, N_("Color nicknames in userlist"), P_OFFINTNL(pchat_gui_ulist_color), N_("Will color nicknames the same as in chat."), 0, 0},
	{ST_TOGGLE, N_("Show user count in channels"), P_OFFINTNL(pchat_gui_ulist_count), 0, 0, 0},
/*	{ST_TOGGLE, N_("Resizable user list"), P_OFFINTNL(pchat_gui_ulist_resizable),0,0,0},*/
	{ST_MENU,	N_("User list sorted by:"), P_OFFINTNL(pchat_gui_ulist_sort), 0, ulmenutext, 0},
	{ST_MENU,	N_("Show user list at:"), P_OFFINTNL(pchat_gui_ulist_pos), 0, ulpos, 1},

	{ST_HEADER,	N_("Away Tracking"),0,0,0},
	{ST_TOGGLE,	N_("Track the Away status of users and mark them in a different color"), P_OFFINTNL(pchat_away_track),0,0,1},
	{ST_NUMBER, N_("On channels smaller than:"), P_OFFINTNL(pchat_away_size_max),0,0,10000},

	{ST_HEADER,	N_("Action Upon Double Click"),0,0,0},
	{ST_ENTRY,	N_("Execute command:"), P_OFFSETNL(pchat_gui_ulist_doubleclick), 0, 0, sizeof prefs.pchat_gui_ulist_doubleclick},

	{ST_HEADER,	N_("Extra Gadgets"),0,0,0},
	{ST_MENU,	N_("Lag meter:"), P_OFFINTNL(pchat_gui_lagometer), 0, lagmenutext, 0},
	{ST_MENU,	N_("Throttle meter:"), P_OFFINTNL(pchat_gui_throttlemeter), 0, lagmenutext, 0},
...
static const setting tabs_settings[] =
{
	...
	{ST_RADIO,  N_("Switcher type:"),P_OFFINTNL(pchat_gui_tab_layout), 0, swtype, 0},
	{ST_TOGGLE, N_("Open an extra tab for server messages"), P_OFFINTNL(pchat_gui_tab_server), 0, 0, 0},
	{ST_TOGGLE, N_("Open a new tab when you receive a private message"), P_OFFINTNL(pchat_gui_autoopen_dialog), 0, 0, 0},
	{ST_TOGGLE, N_("Sort tabs in alphabetical order"), P_OFFINTNL(pchat_gui_tab_sort), 0, 0, 0},
	...
	{ST_MENU,	N_("Show channel switcher at:"), P_OFFINTNL(pchat_gui_tab_pos), 0, cspos, 1},
	{ST_NUMBER,	N_("Shorten tab labels to:"), P_OFFINTNL(pchat_gui_tab_trunc), 0, (const char **)N_("letters."), 99},

	{ST_HEADER,	N_("Tabs or Windows"),0,0,0},
	{ST_MENU,	N_("Open channels in:"), P_OFFINTNL(pchat_gui_tab_chans), 0, tabwin, 0},
	{ST_MENU,	N_("Open dialogs in:"), P_OFFINTNL(pchat_gui_tab_dialogs), 0, tabwin, 0},
	{ST_MENU,	N_("Open utilities in:"), P_OFFINTNL(pchat_gui_tab_utils), N_("Open DCC, Ignore, Notify etc, in tabs or windows?"), tabwin, 0},
...
{
	{ST_TOGGLE, N_("Messages"), P_OFFINTNL(pchat_text_stripcolor_msg), 0, 0, 0},
	{ST_TOGGLE, N_("Scrollback"), P_OFFINTNL(pchat_text_stripcolor_replay), 0, 0, 0},
	{ST_TOGGLE, N_("Topic"), P_OFFINTNL(pchat_text_stripcolor_topic), 0, 0, 0},
...
static const setting filexfer_settings[] =
{
	...
	{ST_MENU,	N_("Auto accept file offers:"), P_OFFINTNL(pchat_dcc_auto_recv), 0, dccaccept, 0},
	{ST_EFOLDER,N_("Download files to:"), P_OFFSETNL(pchat_dcc_dir), 0, 0, sizeof prefs.pchat_dcc_dir},
	{ST_EFOLDER,N_("Move completed files to:"), P_OFFSETNL(pchat_dcc_completed_dir), 0, 0, sizeof prefs.pchat_dcc_completed_dir},
	{ST_TOGGLE, N_("Save nick name in filenames"), P_OFFINTNL(pchat_dcc_save_nick), 0, 0, 0},

	{ST_HEADER,	N_("Auto Open DCC Windows"),0,0,0},
	{ST_TOGGLE, N_("Send window"), P_OFFINTNL(pchat_gui_autoopen_send), 0, 0, 0},
	{ST_TOGGLE, N_("Receive window"), P_OFFINTNL(pchat_gui_autoopen_recv), 0, 0, 0},
	{ST_TOGGLE, N_("Chat window"), P_OFFINTNL(pchat_gui_autoopen_chat), 0, 0, 0},

	{ST_HEADER, N_("Maximum File Transfer Speeds (bytes per second)"), 0, 0, 0},
	{ST_NUMBER,	N_("One upload:"), P_OFFINTNL(pchat_dcc_max_send_cps),
					N_("Maximum speed for one transfer"), 0, 10000000},
	{ST_NUMBER,	N_("One download:"), P_OFFINTNL(pchat_dcc_max_get_cps),
					N_("Maximum speed for one transfer"), 0, 10000000},
	{ST_NUMBER,	N_("All uploads combined:"), P_OFFINTNL(pchat_dcc_global_max_send_cps),
					N_("Maximum speed for all files"), 0, 10000000},
	{ST_NUMBER,	N_("All downloads combined:"), P_OFFINTNL(pchat_dcc_global_max_get_cps),
					N_("Maximum speed for all files"), 0, 10000000},
...
{
	P_OFFINTNL(pchat_input_balloon_chans), P_OFFINTNL(pchat_input_balloon_priv), P_OFFINTNL(pchat_input_balloon_hilight)
};

static const int trayblinklist[3] =
{
	P_OFFINTNL(pchat_input_tray_chans), P_OFFINTNL(pchat_input_tray_priv), P_OFFINTNL(pchat_input_tray_hilight)
};

static const int taskbarlist[3] =
{
	P_OFFINTNL(pchat_input_flash_chans), P_OFFINTNL(pchat_input_flash_priv), P_OFFINTNL(pchat_input_flash_hilight)
};

static const int beeplist[3] =
{
	P_OFFINTNL(pchat_input_beep_chans), P_OFFINTNL(pchat_input_beep_priv), P_OFFINTNL(pchat_input_beep_hilight)
};
...
static const setting alert_settings[] =
{
	...
	{ST_TOGGLE,	N_("Omit alerts when marked as being away"), P_OFFINTNL(pchat_away_omit_alerts), 0, 0, 0},
	{ST_TOGGLE,	N_("Omit alerts while the window is focused"), P_OFFINTNL(pchat_gui_focus_omitalerts), 0, 0, 0},

	{ST_HEADER,	N_("Tray Behavior"), 0, 0, 0},
#ifdef _WIN32
	{ST_TOGGLE,	N_("Enable system tray icon"), P_OFFINTNL(pchat_gui_tray), 0, 0, 3},
#else
	{ST_TOGGLE,	N_("Enable system tray icon"), P_OFFINTNL(pchat_gui_tray), 0, 0, 4},
#endif
	{ST_TOGGLE,	N_("Minimize to tray"), P_OFFINTNL(pchat_gui_tray_minimize), 0, 0, 0},
	{ST_TOGGLE,	N_("Close to tray"), P_OFFINTNL(pchat_gui_tray_close), 0, 0, 0},
	{ST_TOGGLE,	N_("Automatically mark away/back"), P_OFFINTNL(pchat_gui_tray_away), N_("Automatically change status when hiding to tray."), 0, 0},
#ifndef _WIN32
	{ST_TOGGLE,	N_("Only show tray balloons when hidden or iconified"), P_OFFINTNL(pchat_gui_tray_quiet), 0, 0, 0},
#endif

	{ST_HEADER,	N_("Highlighted Messages"),0,0,0},
	{ST_LABEL,	N_("Highlighted messages are ones where your nickname is mentioned, but also:"), 0, 0, 0, 1},

	{ST_ENTRY,	N_("Extra words to highlight:"), P_OFFSETNL(pchat_irc_extra_hilight), 0, 0, sizeof prefs.pchat_irc_extra_hilight},
	{ST_ENTRY,	N_("Nick names not to highlight:"), P_OFFSETNL(pchat_irc_no_hilight), 0, 0, sizeof prefs.pchat_irc_no_hilight},
	{ST_ENTRY,	N_("Nick names to always highlight:"), P_OFFSETNL(pchat_irc_nick_hilight), 0, 0, sizeof prefs.pchat_irc_nick_hilight},
...
static const setting general_settings[] =
{
	...
	{ST_ENTRY,	N_("Quit:"), P_OFFSETNL(pchat_irc_quit_reason), 0, 0, sizeof prefs.pchat_irc_quit_reason},
	{ST_ENTRY,	N_("Leave channel:"), P_OFFSETNL(pchat_irc_part_reason), 0, 0, sizeof prefs.pchat_irc_part_reason},
	{ST_ENTRY,	N_("Away:"), P_OFFSETNL(pchat_away_reason), 0, 0, sizeof prefs.pchat_away_reason},

	{ST_HEADER,	N_("Away"),0,0,0},
	{ST_TOGGLE,	N_("Show away once"), P_OFFINTNL(pchat_away_show_once), N_("Show identical away messages only once."), 0, 0},
	{ST_TOGGLE,	N_("Automatically unmark away"), P_OFFINTNL(pchat_away_auto_unmark), N_("Unmark yourself as away before sending messages."), 0, 0},
...
static const setting advanced_settings[] =
{
	...
	{ST_TOGGLE, N_("Automatically copy selected text"), P_OFFINTNL(pchat_text_autocopy_text),
	...
	{ST_TOGGLE, N_("Automatically include time stamps"), P_OFFINTNL(pchat_text_autocopy_stamp),
	...
	{ST_TOGGLE, N_("Automatically include color information"), P_OFFINTNL(pchat_text_autocopy_color),
	...
	{ST_HEADER,	N_("Miscellaneous"), 0, 0, 0},
	{ST_ENTRY,  N_("Real name:"), P_OFFSETNL(pchat_irc_real_name), 0, 0, sizeof prefs.pchat_irc_real_name},
#ifdef _WIN32
	{ST_ENTRY,  N_("Alternative fonts:"), P_OFFSETNL(pchat_text_font_alternative), N_("Separate multiple entries with commas without spaces before or after."), 0, sizeof prefs.pchat_text_font_alternative},
#endif
	{ST_TOGGLE,	N_("Display lists in compact mode"), P_OFFINTNL(pchat_gui_compact), N_("Use less spacing between user list/channel tree rows."), 0, 0},
	{ST_TOGGLE,	N_("Use server time if supported"), P_OFFINTNL(pchat_irc_cap_server_time), N_("Display timestamps obtained from server if it supports the time-server extension."), 0, 0},
	{ST_TOGGLE,	N_("Automatically reconnect to servers on disconnect"), P_OFFINTNL(pchat_net_auto_reconnect), 0, 0, 1},
	{ST_NUMBER,	N_("Auto reconnect delay:"), P_OFFINTNL(pchat_net_reconnect_delay), 0, 0, 9999},
	{ST_NUMBER,	N_("Auto join delay:"), P_OFFINTNL(pchat_irc_join_delay), 0, 0, 9999},
	{ST_MENU,	N_("Ban Type:"), P_OFFINTNL(pchat_irc_ban_type), N_("Attempt to use this banmask when banning or quieting. (requires irc_who_join)"), bantypemenu, 0},
...
static const setting logging_settings[] =
{
	...
	{ST_TOGGLE,	N_("Display scrollback from previous session"), P_OFFINTNL(pchat_text_replay), 0, 0, 0},
	{ST_NUMBER,	N_("Scrollback lines:"), P_OFFINTNL(pchat_text_max_lines),0,0,100000},
	{ST_TOGGLE,	N_("Enable logging of conversations to disk"), P_OFFINTNL(pchat_irc_logging), 0, 0, 0},
	{ST_ENTRY,	N_("Log filename:"), P_OFFSETNL(pchat_irc_logmask), 0, 0, sizeof prefs.pchat_irc_logmask},
	{ST_LABEL,	N_("%s=Server %c=Channel %n=Network.")},

	{ST_HEADER,	N_("Time Stamps"),0,0,0},
	{ST_TOGGLE,	N_("Insert timestamps in logs"), P_OFFINTNL(pchat_stamp_log), 0, 0, 1},
	{ST_ENTRY,	N_("Log timestamp format:"), P_OFFSETNL(pchat_stamp_log_format), 0, 0, sizeof prefs.pchat_stamp_log_format},
#ifdef _WIN32
	...
	{ST_TOGGLE,	N_("Enable logging of URLs to disk"), P_OFFINTNL(pchat_url_logging), 0, 0, 0},
	{ST_TOGGLE,	N_("Enable URL grabber"), P_OFFINTNL(pchat_url_grabber), 0, 0, 1},
	{ST_NUMBER,	N_("Maximum number of URLs to grab:"), P_OFFINTNL(pchat_url_grabber_limit), 0, 0, 9999},
...
static const setting network_settings[] =
{
	...
	{ST_ENTRY,	N_("Bind to:"), P_OFFSETNL(pchat_net_bind_host), 0, 0, sizeof prefs.pchat_net_bind_host},
	{ST_LABEL,	N_("Only useful for computers with multiple addresses.")},

	{ST_HEADER, N_("File Transfers"), 0, 0, 0},
	{ST_TOGGLE, N_("Get my address from the IRC server"), P_OFFINTNL(pchat_dcc_ip_from_server),
					N_("Asks the IRC server for your real address. Use this if you have a 192.168.*.* address!"), 0, 0},
	{ST_ENTRY,	N_("DCC IP address:"), P_OFFSETNL(pchat_dcc_ip),
					N_("Claim you are at this address when offering files."), 0, sizeof prefs.pchat_dcc_ip},
	{ST_NUMBER,	N_("First DCC send port:"), P_OFFINTNL(pchat_dcc_port_first), 0, 0, 65535},
	{ST_NUMBER,	N_("Last DCC send port:"), P_OFFINTNL(pchat_dcc_port_last), 0,
		(const char **)N_("!Leave ports at zero for full range."), 65535},

	{ST_HEADER,	N_("Proxy Server"), 0, 0, 0, 0},
	{ST_ENTRY,	N_("Hostname:"), P_OFFSETNL(pchat_net_proxy_host), 0, 0, sizeof prefs.pchat_net_proxy_host},
	{ST_NUMBER,	N_("Port:"), P_OFFINTNL(pchat_net_proxy_port), 0, 0, 65535},
	{ST_MENU,	N_("Type:"), P_OFFINTNL(pchat_net_proxy_type), 0, proxytypes, 0},
	{ST_MENU,	N_("Use proxy for:"), P_OFFINTNL(pchat_net_proxy_use), 0, proxyuse, 0},

	{ST_HEADER,	N_("Proxy Authentication"), 0, 0, 0, 0},
#ifdef USE_MSPROXY
	{ST_TOGGLE,	N_("Use Authentication (MS Proxy, HTTP or Socks5 only)"), P_OFFINTNL(pchat_net_proxy_auth), 0, 0, 0},
#else
	{ST_TOGGLE,	N_("Use Authentication (HTTP or Socks5 only)"), P_OFFINTNL(pchat_net_proxy_auth), 0, 0, 0},
#endif
...
setup_create_entry (GtkWidget *table, int row, const setting *set)
{
	...
	if (set->offset == P_OFFSETNL(pchat_net_proxy_user))
		proxy_user = wid;
	if (set->offset == P_OFFSETNL(pchat_net_proxy_pass))
		proxy_pass = wid;

	/* only http and Socks5 can auth */
	if ( (set->offset == P_OFFSETNL(pchat_net_proxy_pass) ||
			set->offset == P_OFFSETNL(pchat_net_proxy_user)) &&
	     (setup_prefs.pchat_net_proxy_type != 4 && setup_prefs.pchat_net_proxy_type != 3 && setup_prefs.pchat_net_proxy_type != 5) )
		gtk_widget_set_sensitive (wid, FALSE);
...
setup_create_frame (GtkWidget **left, GtkWidget *box)
{
	...
	*left = gtk_box_new (GTK_ORIENTATION_VERTICAL, FALSE);
...
setup_create_page (const setting *set)
{
	...
	if (set == general_settings)
	{
		setup_create_id_menu (tab, _("Mark identified users with:"),
									 row, setup_prefs.pchat_irc_id_ytext);
		setup_create_id_menu (tab, _("Mark not-identified users with:"),
									 row + 1, setup_prefs.pchat_irc_id_ntext);
	}
#endif
...
setup_create_sound_page (void)
{
	...
	vbox1 = gtk_box_new (GTK_ORIENTATION_VERTICAL, FALSE);
...
setup_add_page (const char *title, GtkWidget *book, GtkWidget *tab)
{
	...
	vvbox = gtk_box_new (GTK_ORIENTATION_VERTICAL, FALSE);
...
setup_apply_to_sess (session_gui *gui)
{
	...
	sexy_spell_entry_set_checked ((SexySpellEntry *)gui->input_box, prefs.pchat_gui_input_spell);
	sexy_spell_entry_set_parse_attributes ((SexySpellEntry *)gui->input_box, prefs.pchat_gui_input_attr);
}
...
setup_apply_real (int new_pix, int do_ulist, int do_layout)
{
	...
	unslash (prefs.pchat_dcc_dir);
	unslash (prefs.pchat_dcc_completed_dir);

	g_mkdir (prefs.pchat_dcc_dir, 0700);
	g_mkdir (prefs.pchat_dcc_completed_dir, 0700);
...
setup_apply (struct xchatprefs *pr)
{
	...
	if (strcmp (pr->pchat_text_background, prefs.pchat_text_background) != 0)
		new_pix = TRUE;

#define DIFF(a) (pr->a != prefs.a)

#ifdef _WIN32
	if (DIFF (pchat_gui_lang))
		noapply = TRUE;
#endif
	if (DIFF (pchat_gui_compact))
		noapply = TRUE;
	if (DIFF (pchat_gui_input_icon))
		noapply = TRUE;
	if (DIFF (pchat_gui_input_nick))
		noapply = TRUE;
	if (DIFF (pchat_gui_lagometer))
		noapply = TRUE;
	if (DIFF (pchat_gui_tab_icons))
		noapply = TRUE;
	if (DIFF (pchat_gui_tab_server))
		noapply = TRUE;
	if (DIFF (pchat_gui_tab_small))
		noapply = TRUE;
	if (DIFF (pchat_gui_tab_sort))
		noapply = TRUE;
	if (DIFF (pchat_gui_tab_trunc))
		noapply = TRUE;
	if (DIFF (pchat_gui_throttlemeter))
		noapply = TRUE;
	if (DIFF (pchat_gui_ulist_count))
		noapply = TRUE;
	...
	if (DIFF (pchat_gui_tab_dots))
		do_layout = TRUE;
	if (DIFF (pchat_gui_tab_layout))
		do_layout = TRUE;
	...
	if ((pr->pchat_gui_tab_pos == 5 || pr->pchat_gui_tab_pos == 6) &&
		 pr->pchat_gui_tab_layout == 2 && pr->pchat_gui_tab_pos != prefs.pchat_gui_tab_pos)
...
#ifndef _WIN32
	if (prefs.pchat_dcc_auto_recv)
	{
		if (!strcmp ((char *)g_get_home_dir (), prefs.pchat_dcc_dir))
		{
...
#if 0
	wid = gtk_button_new_with_mnemonic ("_Apply");
	g_signal_connect (G_OBJECT (wid), "clicked",
							G_CALLBACK (setup_apply_cb), win);
	gtk_box_pack_start (GTK_BOX (hbbox), wid, FALSE, FALSE, 0);
#endif

	cancel_button = wid = gtk_button_new_with_mnemonic ("_Cancel");
	g_signal_connect (G_OBJECT (wid), "clicked",
