...
gtk_xtext_create_cairo_handle (GtkXText *xtext)
{
	...
	/* If we're in a draw callback, return the provided context directly
	 * Drawing functions will check if this is the draw_cr before destroying */
	if (xtext->draw_cr)
	{
		return xtext->draw_cr;
	}
...
static inline void
xtext_draw_line (GtkXText *xtext, cairo_t *cr, GdkRGBA *color, int x1, int y1, int x2, int y2)
{
	cairo_save (cr);
...
xtext_draw_bg (GtkXText *xtext, int x, int y, int width, int height)
{
	...
	/* Don't destroy if this is the draw callback's context */
	if (cr != xtext->draw_cr)
		cairo_destroy(cr);
}
...
{
	/* Tell GTK this widget does its own drawing */
	gtk_widget_set_app_paintable (GTK_WIDGET (xtext), TRUE);
...
gtk_xtext_adjustment_set (xtext_buffer *buf, int fire_signal)
{
	...
	if (buf->xtext->buffer == buf)
	{
		...
		gtk_adjustment_set_page_size (adj, gtk_widget_get_allocated_height (GTK_WIDGET (buf->xtext))/
							  buf->xtext->fontsize);
...
gtk_xtext_realize (GtkWidget * widget)
{
	...
	xtext->depth = gdk_visual_get_depth (gdk_window_get_visual (gtk_widget_get_window (widget)));

	xtext_set_fg (xtext,  XTEXT_FG);
	xtext_set_bg (xtext,  XTEXT_BG);

	/* Create background pattern from background color */
	if (xtext->bg_pattern)
		cairo_pattern_destroy (xtext->bg_pattern);
	xtext->bg_pattern = cairo_pattern_create_rgba (xtext->bgc.red, xtext->bgc.green, xtext->bgc.blue, xtext->bgc.alpha);
...
gtk_xtext_draw_marker (GtkXText * xtext, textentry * ent, int y)
{
	...
	gdk_cairo_set_source_rgba(cr, &xtext->palette[XTEXT_MARKER]);
...
gtk_xtext_scrolldown_timeout (GtkXText * xtext)
{
	...
	gtk_adjustment_set_value (adj, (int)gtk_adjustment_get_value (adj));	/* Align to line boundary */
...
gtk_xtext_check_mark_stamp (GtkXText *xtext, GdkModifierType mask)
{
	...
	if (mask & STATE_SHIFT || prefs.pchat_text_autocopy_stamp)
	{
		if (!xtext->mark_stamp)
		{
...
gtk_xtext_button_release (GtkWidget * widget, GdkEventButton * event)
{
	...
	if (event->button == 1)
	{
		...
		{
			xtext->color_paste = FALSE;
			if (event->state & STATE_CTRL || prefs.pchat_text_autocopy_color)
...
gtk_xtext_button_press (GtkWidget * widget, GdkEventButton * event)
{
	...
	if (xtext->buffer->last_ent_start)
	{
		...
		/* Force a full redraw to clear old selection */
...
gtk_xtext_class_init (GtkXTextClass * class)
{
	...
	xtext_signals[WORD_CLICK] =
		g_signal_new ("word_click",
							...
							NULL, NULL,
							_pchat_marshal_VOID__POINTER_POINTER,
							G_TYPE_NONE,
...
/*	xtext_signals[SET_SCROLL_ADJUSTMENTS] =
		g_signal_new ("set_scroll_adjustments",
							...
							NULL, NULL,
							_xchat_marshal_VOID__OBJECT_OBJECT,
							G_TYPE_NONE,
...
gtk_xtext_render_page (GtkXText * xtext)
{
	...
	/* Get dimensions from widget allocation if we're in a draw callback */
	if (xtext->draw_cr)
	{
		GtkAllocation allocation;
		gtk_widget_get_allocation (GTK_WIDGET (xtext), &allocation);
