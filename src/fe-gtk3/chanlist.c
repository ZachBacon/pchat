...
chanlist_minusers (GtkSpinButton *wid, server *serv)
{
	...
	prefs.pchat_gui_chanlist_minusers = serv->gui->chanlist_minusers;
	save_config();
...
chanlist_maxusers (GtkSpinButton *wid, server *serv)
{
	...
	prefs.pchat_gui_chanlist_maxusers = serv->gui->chanlist_maxusers;
	save_config();
}
...
chanlist_opengui (server *serv, int do_refresh)
{
	...
	{
		if (prefs.pchat_gui_chanlist_minusers < 1 || prefs.pchat_gui_chanlist_minusers > 999999)
		{
			prefs.pchat_gui_chanlist_minusers = 5;
			save_config();
		}

		serv->gui->chanlist_minusers = prefs.pchat_gui_chanlist_minusers;
	}

	if (!serv->gui->chanlist_maxusers)
	{
		if (prefs.pchat_gui_chanlist_maxusers < 1 || prefs.pchat_gui_chanlist_maxusers > 999999)
		{
			prefs.pchat_gui_chanlist_maxusers = 9999;
			save_config();
		}

		serv->gui->chanlist_maxusers = prefs.pchat_gui_chanlist_maxusers;
	}
	...
	gtk_grid_attach (GTK_GRID (table), wid, 2, 0, 1, 4);
	gtk_widget_show (wid);
