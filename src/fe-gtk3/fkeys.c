...
struct key_binding
{
	char *accel;					  /* String of gtk accelerator, must be freed */
...
static inline int
key_compare_modifiers (GdkModifierType mod1, GdkModifierType mod2)
{
	const GdkModifierType valid_mods = \
						( GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK
						| GDK_SUPER_MASK );

	return ((mod1 & valid_mods) == (mod2 & valid_mods));
}
...
key_handle_key_press (GtkWidget *wid, GdkEventKey *evt, session *sess)
{
	...
	while (list)
	{
		...
		gtk_accelerator_parse (kb->accel, &keyval, &mod);

		if (keyval == evt->keyval && key_compare_modifiers (mod, evt->state))
		{
					...
					GdkModifierType accel_mods, guint hardware_keycode, gpointer userdata)
{
	...
	key_name = gtk_accelerator_name (accel_key, accel_mods);
...
static void
key_dialog_row_activated (GtkTreeSelection *sel, gpointer userdata)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkWidget *textview;
...
key_save_kbs (void)
{
	...
	if (fd < 0)
		return 1;
	write (fd, buf, snprintf (buf, 510, "# PChat key bindings config file\n\n"));

	while (list)
	{
		kb = list->data;

		snprintf (buf, 510, "ACCEL=%s\n%s\n", kb->accel, key_actions[kb->action].name);
...
key_load_kbs (void)
{
	...
	struct stat st;
	struct key_binding *kb = NULL;
	int fd, len, state = 0, pnt = 0, keyval;
	...
	while (buf_get_line (ibuf, &buf, &pnt, size))
	{
		...
		case KBSTATE_KEY:
			...
			kb->accel = gtk_accelerator_name (keyval, mod);
							...
							struct session *sess)
{
	...
	if (ent_start == 0 && text[0] == prefs.pchat_input_command_char[0])
	{
		ent_start++;
		is_cmd = 1;
	}
	...
	else
	{
		if (is_nick)
		{
			...
			if (prefs.pchat_completion_sort == 1)	/* sort in last-talk order? */
		...
		else
		{
			...
			if (prefs.pchat_completion_amount && g_list_length (list) <= prefs.pchat_completion_amount)
			{
				g_free(result);
				result = (char*)list->data;
			}
