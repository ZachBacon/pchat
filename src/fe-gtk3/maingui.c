...
mg_create_tab_colors (void)
{
	...
	plain_list = mg_attr_list_create (NULL, prefs.pchat_gui_tab_small);
	newdata_list = mg_attr_list_create (&colors[COL_NEW_DATA], prefs.pchat_gui_tab_small);
	nickseen_list = mg_attr_list_create (&colors[COL_HILIGHT], prefs.pchat_gui_tab_small);
	newmsg_list = mg_attr_list_create (&colors[COL_NEW_MSG], prefs.pchat_gui_tab_small);
...
mg_windowstate_cb (GtkWindow *wid, GdkEventWindowState *event, gpointer userdata)
{
	if ((event->changed_mask & GDK_WINDOW_STATE_ICONIFIED) &&
		 ...
		 prefs.pchat_gui_tray_minimize && !unity_mode ())
	{
		tray_toggle_visibility (TRUE);
		gtk_window_deiconify (wid);
	}

	prefs.pchat_gui_win_state = 0;
	if (event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED)
		prefs.pchat_gui_win_state = 1;

	prefs.pchat_gui_win_fullscreen = 0;
	if (event->new_window_state & GDK_WINDOW_STATE_FULLSCREEN)
		prefs.pchat_gui_win_fullscreen = 1;

	menu_set_fullscreen (current_sess->gui, prefs.pchat_gui_win_fullscreen);
...
mg_configure_cb (GtkWidget *wid, GdkEventConfigure *event, session *sess)
{
	if (sess == NULL)			/* for the main_window */
	{
		if (mg_gui)
		{
			if (prefs.pchat_gui_win_save && !prefs.pchat_gui_win_state && !prefs.pchat_gui_win_fullscreen)
			{
				sess = current_sess;
				gtk_window_get_position (GTK_WINDOW (wid), &prefs.pchat_gui_win_left,
												 &prefs.pchat_gui_win_top);
				gtk_window_get_size (GTK_WINDOW (wid), &prefs.pchat_gui_win_width,
											&prefs.pchat_gui_win_height);
			}
		}
	}
	...
	if (sess)
	{
		if (sess->type == SESS_DIALOG && prefs.pchat_gui_win_save)
		{
			gtk_window_get_position (GTK_WINDOW (wid), &prefs.pchat_gui_dialog_left,
											 &prefs.pchat_gui_dialog_top);
			gtk_window_get_size (GTK_WINDOW (wid), &prefs.pchat_gui_dialog_width,
										&prefs.pchat_gui_dialog_height);
		}
	}
...
mg_userlist_showhide (session *sess, int show)
{
	...
	right_size = MAX (prefs.pchat_gui_pane_right_size, prefs.pchat_gui_pane_right_size_min);
...
{
	if (prefs.pchat_gui_tab_pos == POS_TOPLEFT && prefs.pchat_gui_ulist_pos == POS_BOTTOMLEFT)
		return TRUE;
	if (prefs.pchat_gui_tab_pos == POS_BOTTOMLEFT && prefs.pchat_gui_ulist_pos == POS_TOPLEFT)
		return TRUE;

	if (prefs.pchat_gui_tab_pos == POS_TOPRIGHT && prefs.pchat_gui_ulist_pos == POS_BOTTOMRIGHT)
		return TRUE;
	if (prefs.pchat_gui_tab_pos == POS_BOTTOMRIGHT && prefs.pchat_gui_ulist_pos == POS_TOPRIGHT)
		return TRUE;
...
mg_populate (session *sess)
{
	...
	case SESS_DIALOG:
		...
		if (prefs.pchat_gui_topicbar)
			gtk_widget_show (gui->topic_bar);
		break;
	case SESS_SERVER:
		if (prefs.pchat_gui_mode_buttons)
...
mg_create_perchannelmenu (session *sess, GtkWidget *menu)
{
	...
	mg_perchan_menu_item (_("_Log to Disk"), submenu, &sess->text_logging, prefs.pchat_irc_logging);
	mg_perchan_menu_item (_("_Reload Scrollback"), submenu, &sess->text_scrollback, prefs.pchat_text_replay);
	if (sess->type == SESS_CHANNEL)
	{
		mg_perchan_menu_item (_("Strip _Colors"), submenu, &sess->text_strip, prefs.pchat_text_stripcolor_msg);
		mg_perchan_menu_item (_("_Hide Join/Part Messages"), submenu, &sess->text_hidejoinpart, prefs.pchat_irc_conf_mode);
	}
}
...
mg_create_alertmenu (session *sess, GtkWidget *menu)
{
	...
	mg_perchan_menu_item (_("Beep on _Message"), submenu, &sess->alert_beep, prefs.pchat_input_beep_chans);

	mg_perchan_menu_item (_("Blink Tray _Icon"), submenu, &sess->alert_tray, prefs.pchat_input_tray_chans);

	mg_perchan_menu_item (_("Blink Task _Bar"), submenu, &sess->alert_taskbar, prefs.pchat_input_flash_chans);
}
...
{
	gtkutil_button (box, "_Close", _("Close this tab/window"),
...
mg_create_topicbar (session *sess, GtkWidget *box)
{
	...
	gui->topic_bar = hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, FALSE);
	...
	gui->topicbutton_box = bbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, FALSE);
	gtk_box_pack_start (GTK_BOX (hbox), bbox, 0, 0, 0);
	mg_create_chanmodebuttons (gui, bbox);

	gui->dialogbutton_box = bbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, FALSE);
	gtk_box_pack_start (GTK_BOX (hbox), bbox, 0, 0, 0);
...
mg_create_meters (session_gui *gui, GtkWidget *parent_box)
{
	...
	gui->meter_box = infbox = box = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
	gtk_box_pack_start (GTK_BOX (parent_box), box, 0, 0, 0);

	if ((prefs.pchat_gui_lagometer & 2) || (prefs.pchat_gui_throttlemeter & 2))
	{
	...
	if (prefs.pchat_gui_lagometer & 2)
	{
	...
	if (prefs.pchat_gui_throttlemeter & 2)
	{
...
{
	prefs.pchat_gui_pane_divider_position = gtk_paned_get_position (pane);
}

static void
mg_leftpane_cb (GtkPaned *pane, GParamSpec *param, session_gui *gui)
{
	prefs.pchat_gui_pane_left_size = gtk_paned_get_position (pane);
}
...
mg_rightpane_cb (GtkPaned *pane, GParamSpec *param, session_gui *gui)
{
	...
	prefs.pchat_gui_pane_right_size = allocation.width - gtk_paned_get_position (pane) - handle_size;
}
...
mg_place_userlist_and_chanview (session_gui *gui)
{
	...
	mg_sanitize_positions (&prefs.pchat_gui_tab_pos, &prefs.pchat_gui_ulist_pos);

	if (gui->chanview)
	{
		pos = prefs.pchat_gui_tab_pos;
...
mg_change_layout (int type)
{
	if (mg_gui)
	{
		...
		if (type == 0 && prefs.pchat_gui_tab_pos != POS_BOTTOM && prefs.pchat_gui_tab_pos != POS_TOP)
			prefs.pchat_gui_tab_pos = POS_BOTTOM;
...
mg_create_search(session *sess, GtkWidget *box)
{
	...
	gtk_button_set_image (GTK_BUTTON (close), gtk_image_new_from_icon_name ("_Close", GTK_ICON_SIZE_BUTTON));
	...
	gtk_button_set_image (GTK_BUTTON (previous), gtk_image_new_from_icon_name ("_Back", GTK_ICON_SIZE_BUTTON));
	...
	next = gtk_button_new ();
	gtk_button_set_image (GTK_BUTTON (next), gtk_image_new_from_icon_name ("_Forward", GTK_ICON_SIZE_BUTTON));
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(highlight), prefs.pchat_text_search_highlight_all);
	gtk_widget_set_can_focus (highlight, FALSE);
	g_signal_connect (G_OBJECT (highlight), "toggled", G_CALLBACK (search_set_option), &prefs.pchat_text_search_highlight_all);
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(matchcase), prefs.pchat_text_search_case_match);
	gtk_widget_set_can_focus (matchcase, FALSE);
	g_signal_connect (G_OBJECT (matchcase), "toggled", G_CALLBACK (search_set_option), &prefs.pchat_text_search_case_match);
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(regex), prefs.pchat_text_search_regexp);
	gtk_widget_set_can_focus (regex, FALSE);
	g_signal_connect (G_OBJECT (regex), "toggled", G_CALLBACK (search_set_option), &prefs.pchat_text_search_regexp);
...
mg_create_entry (session *sess, GtkWidget *box)
{
	...
	gui->nick_box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, FALSE);
	...
	sexy_spell_entry_set_checked ((SexySpellEntry *)entry, prefs.pchat_gui_input_spell);
	sexy_spell_entry_set_parse_attributes ((SexySpellEntry *)entry, prefs.pchat_gui_input_attr);
	...
	gtk_widget_set_name (entry, "pchat-inputbox");
...
mg_create_tabs (session_gui *gui)
{
	...
	if (prefs.pchat_gui_tab_icons && (pix_tree_channel || pix_tree_dialog || pix_tree_server || pix_tree_util))
	{
		use_icons = TRUE;
	}
...
mg_create_topwindow (session *sess)
{
	...
	if (sess->type == SESS_DIALOG)
		win = gtkutil_window_new ("PChat", NULL,
										  prefs.pchat_gui_dialog_width, prefs.pchat_gui_dialog_height, 0);
	else
		win = gtkutil_window_new ("PChat", NULL,
										  prefs.pchat_gui_win_width,
...
mg_tabwindow_de_cb (GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
	...
	if (prefs.pchat_gui_tray_close && !unity_mode () && tray_toggle_visibility (FALSE))
		return TRUE;
...
fe_clear_channel (session *sess)
{
	...
	if (sess->gui->is_tab)
	{
		...
		{
			if (prefs.pchat_gui_tab_trunc > 2 && g_utf8_strlen (sess->waitchannel, -1) > prefs.pchat_gui_tab_trunc)
			{
...
fe_set_channel (session *sess)
{
	if (sess->res->tab != NULL)
		chan_rename (sess->res->tab, sess->channel, prefs.pchat_gui_tab_trunc);
}
							  ...
							  void *family)
{
	...
	if (prefs.pchat_gui_tab_pos == POS_HIDDEN && prefs.pchat_gui_tab_utils)
		prefs.pchat_gui_tab_utils = 0;

	if (force_toplevel || !prefs.pchat_gui_tab_utils)
	{
...
mg_drag_drop_cb (GtkWidget *widget, GdkDragContext *context, int x, int y, guint time, gpointer user_data)
{
	...
	case GDK_ACTION_MOVE:
		/* from userlist */
		mg_handle_drop (widget, y, &prefs.pchat_gui_ulist_pos, &prefs.pchat_gui_tab_pos);
		break;
	case GDK_ACTION_COPY:
		/* from tree - we use GDK_ACTION_COPY for the tree */
		mg_handle_drop (widget, y, &prefs.pchat_gui_tab_pos, &prefs.pchat_gui_ulist_pos);
		break;
	default:
		return FALSE;
	}
