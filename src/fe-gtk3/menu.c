...
menu_bar_toggle (void)
{
	...
	menu_setting_foreach (menu_showhide_cb, MENU_ID_MENUBAR, !prefs.pchat_gui_hide_menu);
}
...
menu_fullscreen_toggle (GtkWidget *wid, gpointer ud)
{
	...
	else
	{
		...
		gtk_window_move (GTK_WINDOW(parent_window),
					prefs.pchat_gui_win_left, prefs.pchat_gui_win_top);
#endif
	}
}
...
{
	int old = prefs.pchat_gui_tab_chans;
	int oldf = prefs.pchat_gui_tab_newtofront;

	prefs.pchat_gui_tab_chans = 1;
	/* force focus if setting is "only requested tabs" */
	if (prefs.pchat_gui_tab_newtofront == 2)
		prefs.pchat_gui_tab_newtofront = 1;
	new_ircwindow (NULL, NULL, SESS_SERVER, 0);
	prefs.pchat_gui_tab_chans = old;
	prefs.pchat_gui_tab_newtofront = oldf;
}
						...
						GtkWidget **menu_widgets)
{
	...
	mymenu[MENUBAR_OFFSET].state = !prefs.pchat_gui_hide_menu;
	mymenu[MENUBAR_OFFSET+1].state = prefs.pchat_gui_topicbar;
	mymenu[MENUBAR_OFFSET+2].state = !prefs.pchat_gui_ulist_hide;
	mymenu[MENUBAR_OFFSET+3].state = prefs.pchat_gui_ulist_buttons;
	mymenu[MENUBAR_OFFSET+4].state = prefs.pchat_gui_mode_buttons;

	mymenu[AWAY_OFFSET].state = away;

	switch (prefs.pchat_gui_tab_layout)
	{
	case 0:
	...
	{
		item = NULL;
		if (mymenu[i].id == MENU_ID_USERMENU && !prefs.pchat_gui_usermenu)
		{
			i++;
			continue;
		}
