...
#include "util.h"
#include "fe.h"
#include "cfgfiles.h"			  /* pchat_fopen_file() */
...
random_line (char *file_name)
{
	...
	fh = pchat_fopen_file (file_name, "r", 0);
	if (!fh)
	{
	 nofile:
...
server_sendpart (server * serv, char *channel, char *reason)
{
	if (!reason)
	{
		reason = random_line (prefs.pchat_irc_part_reason);
...
server_sendquit (session * sess)
{
	...
	if (!sess->quitreason)
	{
		colrea = g_strdup (prefs.pchat_irc_quit_reason);
...
cmd_away (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (!(*reason))
	{
		...
		if (sess->server->reconnect_away)
			...
			reason = random_line (prefs.pchat_away_reason);
	}
...
cmd_dcc (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (*type)
	{
		...
		if ((!g_ascii_strcasecmp (type, "SEND")) || (!g_ascii_strcasecmp (type, "PSEND")))
		{
			...
			maxcps = prefs.pchat_dcc_max_send_cps;
...
exec_print_line (session *sess, char *data, gssize len, gboolean tochannel)
{
	...
	if (tochannel)
	{
		...
		const unsigned int old = prefs.pchat_completion_auto;
		prefs.pchat_completion_auto = 0;
		handle_multiline (sess, valid, FALSE, TRUE);
		prefs.pchat_completion_auto = old;
	}
	else
...
{
	/* this is pretty much the same as in pchat_exit() */
	save_config ();
...
load_perform_file (session *sess, char *file)
{
	...
	fp = pchat_fopen_file (file, "r", 0);		/* load files from config dir */
	if (!fp)
		return FALSE;
	...
	while (fgets (tbuf, 1024, fp))
	{
		...
		if (nl)
			*nl = 0;
		if (tbuf[0] == prefs.pchat_input_command_char[0])
...
{
	int tmp = prefs.pchat_net_reconnect_delay;
	GSList *list;
	server *serv = sess->server;

	prefs.pchat_net_reconnect_delay = 0;
	...
	prefs.pchat_net_reconnect_delay = tmp;
...
cmd_tray (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (!word[3][0])
	{
		fe_tray_set_file (NULL);	/* default PChat icon */
		return TRUE;
	}
...
#endif
#if 0
	{"EXPORTCONF", cmd_exportconf, 0, 0, 1, N_("EXPORTCONF, exports PChat settings")},
#endif
...
perform_nick_completion (struct session *sess, char *cmd, char *tbuf)
{
	...
	if (space && space != cmd)
	{
		if (space[-1] == prefs.pchat_completion_suffix[0] && space - 1 != cmd)
		{
			len = space - cmd - 1;
			if (len < NICKLEN)
			{
...
/* handle text entered without a pchat_input_command_char prefix */
...
handle_say (session *sess, char *text, int check_spch)
{
	...
	if (check_spch && prefs.pchat_input_perc_color)
		check_special_chars (text, prefs.pchat_input_perc_ascii);
	...
	if (prefs.pchat_completion_auto)
	...
	while (strchr (cmd, '%') != NULL)
	{
		...
		switch (cmd[0])
		{
			case 'n':
				...
				{
					g_string_append (expanded, prefs.pchat_irc_nick1);
				}
				cmd++;
				break;
			...
			case 'r':
				...
				{
					g_string_append (expanded, prefs.pchat_irc_real_name);
				}
				cmd++;
				break;
			...
			case 'u':
				...
				{
					g_string_append (expanded, prefs.pchat_irc_user_name);
				}
				cmd++;
				break;
...
handle_command (session *sess, char *cmd, int check_spch)
{
	...
	if (check_spch && prefs.pchat_input_perc_color)
	{
		check_special_chars (cmd, prefs.pchat_input_perc_ascii);
	}
...
handle_user_input (session *sess, char *text, int history, int nocommand)
{
	...
	if (nocommand || text[0] != prefs.pchat_input_command_char[0])
	{
	...
	/* check for // */
	if (text[0] == prefs.pchat_input_command_char[0] && text[1] == prefs.pchat_input_command_char[0])
	{
	...
	if (prefs.pchat_input_command_char[0] == '/')
	{
		int i;
