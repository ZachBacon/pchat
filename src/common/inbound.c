...
inbound_make_idtext (server *serv, char *idtext, int max, int id)
{
	...
	if (serv->have_idmsg || serv->have_accnotify)
	{
		if (id)
		{
			safe_strcpy (idtext, prefs.pchat_irc_id_ytext, max);
		} else
		{
			safe_strcpy (idtext, prefs.pchat_irc_id_ntext, max);
		}
					  ...
					  const message_tags_data *tags_data)
{
	...
	if (sess || prefs.pchat_gui_autoopen_dialog)
	{
		/*0=ctcp  1=priv will set pchat_gui_autoopen_dialog=0 here is flud detected */
		if (!sess)
		{
		...
		if (ip && ip[0])
		{
			if (prefs.pchat_irc_logging && sess->logfd != -1 &&
...
is_hilight (char *from, char *text, session *sess, server *serv)
{
	...
	if (alert_match_text (text, serv->nick) ||
		 alert_match_text (text, prefs.pchat_irc_extra_hilight) ||
		 alert_match_word (from, prefs.pchat_irc_nick_hilight))
	{
		g_free (text);
		if (sess != current_tab)
		{
					  ...
					  const message_tags_data *tags_data)
{
	...
	if (fromme)
	{
		if (prefs.pchat_away_auto_unmark && serv->is_away && !tags_data->timestamp)
					 ...
					 const message_tags_data *tags_data)
{
	...
	{
		ptr = 0;
		if (prefs.pchat_irc_notice_pos == 0)
		{
											/* paranoia check */
		...
		} else if (prefs.pchat_irc_notice_pos == 1)
		{
				  ...
				  const message_tags_data *tags_data)
{
	...
	{
		if (prefs.pchat_away_show_once && !serv->inside_whois)
			return;
	} else
	{
						 ...
						 const message_tags_data *tags_data)
{
	...
	case 3:
		serv->p_change_nick (serv, prefs.pchat_irc_nick3);
		if (error)
		{
			EMIT_SIGNAL_TIMESTAMP (XP_TE_NICKERROR, sess, nick, prefs.pchat_irc_nick3,
										  NULL, NULL, 0, tags_data->timestamp);
		}
		else
		{
			EMIT_SIGNAL_TIMESTAMP (XP_TE_NICKCLASH, sess, nick, prefs.pchat_irc_nick3,
										  NULL, NULL, 0, tags_data->timestamp);
		}
		break;
...
inbound_login_end (session *sess, char *text, const message_tags_data *tags_data)
{
	...
	if (!serv->end_of_motd)
	{
		if (prefs.pchat_dcc_ip_from_server && serv->use_who)
		{
		...
		if (serv->network && prefs.pchat_irc_join_delay
			&& ((((ircnet *)serv->network)->pass && inbound_nickserv_login (serv))
				|| ((ircnet *)serv->network)->commandlist))
		{
			serv->joindelay_tag = fe_timeout_add (prefs.pchat_irc_join_delay * 1000, check_autojoin_channels, serv);
		}
		else
		{
	...
	if (prefs.pchat_irc_skip_motd && !serv->motd_skipped)
	{
					 ...
					 const message_tags_data *tags_data)
{
	...
	for (i=0; extensions[i]; i++)
	{
		...
		if (prefs.pchat_irc_cap_server_time
...
inbound_sasl_authenticate (server *serv, char *data)
{
		...
		if (net->user && !(net->flags & FLAG_USE_GLOBAL))
			...
			user = prefs.pchat_irc_user_name;
