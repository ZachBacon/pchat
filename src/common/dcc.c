...
static double
timeval_diff (gint64 greater, gint64 less)
{
	/* Both are microseconds since epoch */
	return (greater - less) / 1000000.0;
}
...
dcc_calc_cps (struct DCC *dcc)
{
	...
	if (dcc->type == TYPE_SEND)
	{
		...
		glob_limit = prefs.pchat_dcc_global_max_send_cps;
	}
	else
	{
		pos = dcc->pos;
...
dcc_check_timeouts (void)
{
	...
	while (list)
	{
		...
		case STAT_ACTIVE:
			...
			{
				if (prefs.pchat_dcc_stall_timeout > 0)
				{
					if (!dcc->throttled
						&& tim - dcc->lasttime > prefs.pchat_dcc_stall_timeout)
					{
		...
		case STAT_QUEUED:
			...
			{
				if (tim - dcc->offertime > prefs.pchat_dcc_timeout)
				{
					if (prefs.pchat_dcc_timeout > 0)
					{
...
#define DCC_USE_PROXY() (prefs.pchat_net_proxy_host[0] && prefs.pchat_net_proxy_type>0 && prefs.pchat_net_proxy_type<5 && prefs.pchat_net_proxy_use!=1)
...
dcc_connect_sok (struct DCC *dcc)
{
	...
	if (DCC_USE_PROXY ())
	{
		if (!dcc_lookup_proxy (prefs.pchat_net_proxy_host, &addr))
		{
			closesocket (sok);
			return -1;
		}
		addr.sin_port = htons (prefs.pchat_net_proxy_port);
	}
	else
	{
...
dcc_connect_finished (GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
	...
	case TYPE_SEND:
		/* passive send */
		dcc->fastsend = prefs.pchat_dcc_fast_send;
		if (dcc->fastsend)
...
dcc_socks5_proxy_traverse (GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
	...
	if (proxy->phase == 2)
	{
		...
		if (auth)
		{
			...
			memcpy (proxy->buffer + 2, prefs.pchat_net_proxy_user, len_u);
			proxy->buffer[2 + len_u] = len_p;
			memcpy (proxy->buffer + 3 + len_u, prefs.pchat_net_proxy_pass, len_p);
...
dcc_http_proxy_traverse (GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
	...
	if (proxy->phase == 0)
	{
		...
		if (prefs.pchat_net_proxy_auth)
		{
			n2 = g_snprintf (auth_data2, sizeof (auth_data2), "%s:%s",
							prefs.pchat_net_proxy_user, prefs.pchat_net_proxy_pass);
...
dcc_proxy_connect (GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
	...
	switch (prefs.pchat_net_proxy_type)
	{
...
dcc_send_data (GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
	...
	if (prefs.pchat_dcc_blocksize < 1) /* this is too little! */
		prefs.pchat_dcc_blocksize = 1024;

	if (prefs.pchat_dcc_blocksize > 102400)	/* this is too much! */
		prefs.pchat_dcc_blocksize = 102400;
	...
	buf = malloc (prefs.pchat_dcc_blocksize);
	if (!buf)
		return TRUE;

	lseek (dcc->fp, dcc->pos, SEEK_SET);
	len = read (dcc->fp, buf, prefs.pchat_dcc_blocksize);
	if (len < 1)
		goto abortit;
...
dcc_get_my_address (void)	/* the address we'll tell the other person */
{
	...
	if (prefs.pchat_dcc_ip_from_server && prefs.dcc_ip)
		addr = prefs.dcc_ip;
	else if (prefs.pchat_dcc_ip[0])
	{
	   dns_query = gethostbyname ((const char *) prefs.pchat_dcc_ip);
...
dcc_listen_init (struct DCC *dcc, session *sess)
{
	...
	if (prefs.pchat_dcc_port_first > 0)
	{
		SAddr.sin_port = 0;
		i = 0;
		while ((prefs.pchat_dcc_port_last > ntohs(SAddr.sin_port)) &&
				(bindretval == -1))
		{
			SAddr.sin_port = htons (prefs.pchat_dcc_port_first + i);
			i++;
	...
	/*else we try to take an address from pchat_dcc_ip*/
		...
		if (dcc->fp != -1)
		{
			if (passive || dcc_listen_init (dcc, sess))
			{
				...
				while (*file)
				{
					if (*file == ' ')
					{
						if (prefs.pchat_dcc_send_fillspaces)
				    		*file = '_';
					  	else
					   	havespaces = 1;
					}
...
is_same_file (struct DCC *dcc, struct DCC *new_dcc)
{
	...
	fi_a = g_file_query_info (gf_a, G_FILE_ATTRIBUTE_ID_FILE G_FILE_ATTRIBUTE_ID_FILESYSTEM, G_FILE_QUERY_INFO_NONE, NULL, NULL);
	if (!fi_a)
	{
		g_object_unref (gf_a);
		return FALSE;
	...
	fi_b = g_file_query_info (gf_b, G_FILE_ATTRIBUTE_ID_FILE G_FILE_ATTRIBUTE_ID_FILESYSTEM, G_FILE_QUERY_INFO_NONE, NULL, NULL);
	if (!fi_b)
	{
		g_object_unref (gf_b);
		g_object_unref (fi_a);
...
dcc_get (struct DCC *dcc)
{
	...
	case STAT_QUEUED:
		...
		{
			if (dcc->type == TYPE_RECV && prefs.pchat_dcc_auto_resume && dcc->resumable)
			{
				dcc_resume (dcc);
			}
			else
...
dcc_add_file (session *sess, char *file, DCC_SIZE size, int port, char *nick, guint32 addr, int pasvid)
{
	...
	if (dcc)
	{
		...
		if (prefs.pchat_dcc_auto_recv == 1)
		{
		...
		else if (prefs.pchat_dcc_auto_recv == 2)
		{
			dcc_get (dcc);
		}
		if (prefs.pchat_gui_autoopen_recv)
		{
