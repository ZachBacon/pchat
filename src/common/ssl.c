...
_SSL_context_init (void (*info_cb_func))
{
	...
	/* In OpenSSL < 1.1.0, we need to explicitly initialize */
	SSLeay_add_ssl_algorithms ();
	SSL_load_error_strings ();
	ctx = SSL_CTX_new (SSLv23_client_method ());
#else
	/* OpenSSL 1.1.0+ and 3.x auto-initialize, use TLS_client_method */
...
#if OPENSSL_VERSION_NUMBER >= 0x00908000L && OPENSSL_VERSION_NUMBER < 0x10100000L && !defined (OPENSSL_NO_COMP)
	/* Workaround for OpenSSL 0.9.8 - not needed in 1.1.0+ */
...
#if OPENSSL_VERSION_NUMBER < 0x10100000L && !defined(HAVE_SSL_CTX_GET_SSL_METHOD)
	method = ctx->method;
#else
	method = SSL_CTX_get_ssl_method (ctx);
#endif
	/* In OpenSSL 1.1+, TLS_client_method is used instead of SSLv23_client_method */
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
...
#endif
#endif
	/* OpenSSL 1.1+ and 3.x handle cleanup automatically */
}
...
_SSL_check_subject_altname (X509 *cert, const char *host)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		if (type == GEN_DNS)
		{
			...
			{
#if OPENSSL_VERSION_NUMBER >= 0x10100000L || defined(HAVE_ASN1_STRING_GET0_DATA)
