...
irc_init (session *sess)
{
	...
	if (prefs.pchat_notify_timeout)
		notify_tag = fe_timeout_add (prefs.pchat_notify_timeout * 1000,
											  notify_checklist, 0);

	fe_timeout_add (prefs.pchat_away_timeout * 1000, away_check, 0);
...
set_locale (void)
{
	...
	if (0 <= prefs.pchat_gui_lang && prefs.pchat_gui_lang < LANGUAGES_LENGTH)
		strcat (xchat_lang, languages[prefs.pchat_gui_lang]);
	else
		strcat (xchat_lang, "en");
