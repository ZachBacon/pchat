...
/* shrink the file to roughly prefs.pchat_text_max_lines */
...
scrollback_shrink (session *sess)
{
	...
	gsize len;
	gint offset, lines = 0;
	const gint max_lines = MIN(prefs.pchat_text_max_lines, SCROLLBACK_MAX);
...
scrollback_save (session *sess, char *text, time_t stamp)
{
	...
	if (sess->type == SESS_SERVER && prefs.pchat_gui_tab_server == 1)
		return;
	...
	if ((sess->scrollwritten > prefs.pchat_text_max_lines && prefs.pchat_text_max_lines > 0) ||
...
log_write (session *sess, char *text, time_t ts)
{
	...
	if (prefs.pchat_stamp_log)
	{
		if (!ts) ts = time(0);
		len = get_stamp_str (prefs.pchat_stamp_log_format, ts, &stamp);
		if (len)
		{
...
pevent_make_pntevts (void)
{
	...
	for (i = 0; i < NUM_XP; i++)
	{
		...
		if (pevt_build_string (pntevts_text[i], &(pntevts[i]), &m) != 0)
		{
			...
			{
				g_error ("PChat CRITICAL *** default event text failed to build!");
			}
			else
			{
...
pevent_load (char *filename)
{
	...
	if (filename == NULL)
		fd = pchat_open_file ("pevents.conf", O_RDONLY, 0, 0);
	else
		fd = pchat_open_file (filename, O_RDONLY, 0, XOF_FULLPATH);
...
/*
	...
	1) if prefs.pchat_text_stripcolor_msg is set, filter all style control codes from arguments
...
format_event (session *sess, int index, char **args, char *o, gsize sizeofo, unsigned int stripcolor_args)
{
	...
	while (done_all == FALSE)
	{
		...
		case 1:
			...
			{
				fprintf (stderr,
							"PChat DEBUG: display_event: arg > numargs (%d %d %s)\n",
							a, numargs, i);
				break;
			}
			ar = args[(int) a + 1];
			  ...
			  time_t timestamp)
{
	...
	unsigned int stripcolor_args = (chanopt_is_set (prefs.pchat_text_stripcolor_msg, sess->text_strip) ? 0xFFFFFFFF : 0);
	char tbuf[NICKLEN + 4];

	if (a != NULL && prefs.pchat_text_color_nicks && (index == XP_TE_CHANACTION || index == XP_TE_CHANMSG))
	{
	...
	case XP_TE_QUIT:
		...
		if (chanopt_is_set (prefs.pchat_irc_conf_mode, sess->text_hidejoinpart))
			return;
		break;
	...
	case XP_TE_DPRIVACTION:
		if (chanopt_is_set (prefs.pchat_input_beep_priv, sess->alert_beep) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
			sound_beep (sess);
		if (chanopt_is_set (prefs.pchat_input_flash_priv, sess->alert_taskbar) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
	...
	case XP_TE_HCHANMSG:
		if (chanopt_is_set (prefs.pchat_input_beep_hilight, sess->alert_beep) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
			sound_beep (sess);
		if (chanopt_is_set (prefs.pchat_input_flash_hilight, sess->alert_taskbar) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
			fe_flash_window (sess);
		if (sess->alert_tray == SET_ON)
			fe_tray_set_icon (FE_ICON_MESSAGE);
		break;
	...
	case XP_TE_CHANMSG:
		if (chanopt_is_set (prefs.pchat_input_beep_chans, sess->alert_beep) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
			sound_beep (sess);
		if (chanopt_is_set (prefs.pchat_input_flash_chans, sess->alert_taskbar) && (!prefs.pchat_away_omit_alerts || !sess->server->is_away))
			fe_flash_window (sess);
		if (sess->alert_tray == SET_ON)
			fe_tray_set_icon (FE_ICON_MESSAGE);
		break;
	...
	case XP_TE_CHANGENICK:
		if (prefs.pchat_irc_hide_nickchange)
			return;
		break;
	}

	if (!prefs.pchat_away_omit_alerts || !sess->server->is_away)
...
pevent_save (char *fn)
{
	...
	if (!fn)
		fd = pchat_open_file ("pevents.conf", O_CREAT | O_TRUNC | O_WRONLY,
									 0x180, XOF_DOMODE);
	else
		fd = pchat_open_file (fn, O_CREAT | O_TRUNC | O_WRONLY, 0x180,
...
{
	if (!prefs.pchat_gui_focus_omitalerts || fe_gui_info (sess, 0) != 1)
	{
	...
	if (g_access (wavfile, R_OK) == 0)
	{
		...
		if (ca_con == NULL)
		{
			...
			ca_context_change_props (ca_con,
											CA_PROP_APPLICATION_ID, "pchat",
											CA_PROP_APPLICATION_NAME, "PChat",
											CA_PROP_APPLICATION_ICON_NAME, "pchat", NULL);
		}
...
sound_load ()
{
	...
	fd = pchat_open_file ("sound.conf", O_RDONLY, 0, 0);
	if (fd == -1)
		return;
...
sound_save ()
{
	...
	fd = pchat_open_file ("sound.conf", O_CREAT | O_TRUNC | O_WRONLY, 0x180,
								 XOF_DOMODE);
	if (fd == -1)
		return;
