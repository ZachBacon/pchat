...
{
	g_return_val_if_fail (responses != NULL, PCHAT_EAT_ALL);
	...
	if (word[2] && *word[2] && word[3] && *word[3])
	{
		...
		if (port && port <= G_MAXUINT16)
		{
			...
			pchat_hook_timer (ph, 30000, identd_cleanup_response_cb, GINT_TO_POINTER (port));
		}
	}
	else
	{
		pchat_command (ph, "HELP IDENTD");
	}
		...
		else
		{
			...
			else
			{
				...
				if ((sok_addr = g_socket_connection_get_remote_address (info->conn, NULL)))
				{
					...
					pchat_printf (ph, _("*\tServicing ident request from %s as %s"), addr, info->username);
...
identd_start_server (void)
{
	...
	if (pchat_get_prefs (ph, "identd_server", NULL, &enabled) == 3)
	{
		if (!enabled)
			return;
	}
	if (pchat_get_prefs (ph, "identd_port", NULL, &port) == 2 && (port <= 0 || port > G_MAXUINT16))
	{
		port = 113;
	}
	...
	if (error)
	{
		pchat_printf (ph, _("*\tError starting identd server: %s"), error->message);

		g_error_free (error);
		g_clear_object (&service);
		return;
	}
	/*pchat_printf (ph, "*\tIdentd listening on port: %d", port); */
...
int
identd_plugin_init (pchat_plugin *plugin_handle, char **plugin_name,
					char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = "";
	...
	pchat_hook_command (ph, "IDENTD", PCHAT_PRI_NORM, identd_command_cb,
