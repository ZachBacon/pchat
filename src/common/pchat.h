...
struct pchatprefs
{
	/* these are the rebranded, consistent, sorted pchat variables */

	/* BOOLEANS */
	unsigned int pchat_away_auto_unmark;
	unsigned int pchat_away_omit_alerts;
	unsigned int pchat_away_show_once;
	unsigned int pchat_away_track;
	unsigned int pchat_completion_auto;
	unsigned int pchat_dcc_auto_chat;
	unsigned int pchat_dcc_auto_resume;
	unsigned int pchat_dcc_fast_send;
	unsigned int pchat_dcc_ip_from_server;
	unsigned int pchat_dcc_remove;
	unsigned int pchat_dcc_save_nick;
	unsigned int pchat_dcc_send_fillspaces;
	unsigned int pchat_gui_autoopen_chat;
	unsigned int pchat_gui_autoopen_dialog;
	unsigned int pchat_gui_autoopen_recv;
	unsigned int pchat_gui_autoopen_send;
	unsigned int pchat_gui_compact;
	unsigned int pchat_gui_filesize_iec;
	unsigned int pchat_gui_focus_omitalerts;
	unsigned int pchat_gui_hide_menu;
	unsigned int pchat_gui_input_attr;
	unsigned int pchat_gui_input_icon;
	unsigned int pchat_gui_input_nick;
	unsigned int pchat_gui_input_spell;
	unsigned int pchat_gui_input_style;
	unsigned int pchat_gui_join_dialog;
	unsigned int pchat_gui_mode_buttons;
	unsigned int pchat_gui_quit_dialog;
	/* unsigned int pchat_gui_single; */
	unsigned int pchat_gui_slist_fav;
	unsigned int pchat_gui_slist_skip;
	unsigned int pchat_gui_tab_chans;
	unsigned int pchat_gui_tab_dialogs;
	unsigned int pchat_gui_tab_dots;
	unsigned int pchat_gui_tab_icons;
	unsigned int pchat_gui_tab_scrollchans;
	unsigned int pchat_gui_tab_server;
	unsigned int pchat_gui_tab_sort;
	unsigned int pchat_gui_tab_utils;
	unsigned int pchat_gui_topicbar;
	unsigned int pchat_gui_tray;
	unsigned int pchat_gui_tray_away;
	unsigned int pchat_gui_tray_blink;
	unsigned int pchat_gui_tray_close;
	unsigned int pchat_gui_tray_minimize;
	unsigned int pchat_gui_tray_quiet;
	unsigned int pchat_gui_ulist_buttons;
	unsigned int pchat_gui_ulist_color;
	unsigned int pchat_gui_ulist_count;
	unsigned int pchat_gui_ulist_hide;
	unsigned int pchat_gui_ulist_icons;
	unsigned int pchat_gui_ulist_show_hosts;
	unsigned int pchat_gui_ulist_style;
	unsigned int pchat_gui_usermenu;
	unsigned int pchat_gui_win_modes;
	unsigned int pchat_gui_win_nick;
	unsigned int pchat_gui_win_save;
	unsigned int pchat_gui_win_swap;
	unsigned int pchat_gui_win_ucount;
	unsigned int pchat_identd_server;
	unsigned int pchat_input_balloon_chans;
	unsigned int pchat_input_balloon_hilight;
	unsigned int pchat_input_balloon_priv;
	unsigned int pchat_input_beep_chans;
	unsigned int pchat_input_beep_hilight;
	unsigned int pchat_input_beep_priv;
	unsigned int pchat_input_filter_beep;
	unsigned int pchat_input_flash_chans;
	unsigned int pchat_input_flash_hilight;
	unsigned int pchat_input_flash_priv;
	unsigned int pchat_input_perc_ascii;
	unsigned int pchat_input_perc_color;
	unsigned int pchat_input_tray_chans;
	unsigned int pchat_input_tray_hilight;
	unsigned int pchat_input_tray_priv;
	unsigned int pchat_irc_auto_rejoin;
	unsigned int pchat_irc_reconnect_rejoin;
	unsigned int pchat_irc_conf_mode;
	unsigned int pchat_irc_hidehost;
	unsigned int pchat_irc_hide_nickchange;
	unsigned int pchat_irc_hide_version;
	unsigned int pchat_irc_invisible;
	unsigned int pchat_irc_logging;
	unsigned int pchat_irc_raw_modes;
	unsigned int pchat_irc_servernotice;
	unsigned int pchat_irc_skip_motd;
	unsigned int pchat_irc_wallops;
	unsigned int pchat_irc_who_join;
	unsigned int pchat_irc_whois_front;
	unsigned int pchat_irc_cap_server_time;
	unsigned int pchat_net_auto_reconnect;
	unsigned int pchat_net_auto_reconnectonfail;
	unsigned int pchat_net_proxy_auth;
	unsigned int pchat_net_throttle;
	unsigned int pchat_notify_whois_online;
	unsigned int pchat_perl_warnings;
	unsigned int pchat_stamp_log;
	unsigned int pchat_stamp_text;
	unsigned int pchat_text_autocopy_color;
	unsigned int pchat_text_autocopy_stamp;
	unsigned int pchat_text_autocopy_text;
	unsigned int pchat_text_color_nicks;
	unsigned int pchat_text_indent;
	unsigned int pchat_text_replay;
	unsigned int pchat_text_search_case_match;
	unsigned int pchat_text_search_highlight_all;
	unsigned int pchat_text_search_follow;
	unsigned int pchat_text_search_regexp;
	unsigned int pchat_text_show_marker;
	unsigned int pchat_text_show_sep;
	unsigned int pchat_text_stripcolor_msg;
	unsigned int pchat_text_stripcolor_replay;
	unsigned int pchat_text_stripcolor_topic;
	unsigned int pchat_text_thin_sep;
	unsigned int pchat_text_transparent;
	unsigned int pchat_text_wordwrap;
	unsigned int pchat_url_grabber;
	unsigned int pchat_url_logging;

	/* NUMBERS */
	int pchat_away_size_max;
	int pchat_away_timeout;
	int pchat_completion_amount;
	int pchat_completion_sort;
	int pchat_dcc_auto_recv;
	int pchat_dcc_blocksize;
	int pchat_dcc_global_max_get_cps;
	int pchat_dcc_global_max_send_cps;
	int pchat_dcc_max_get_cps;
	int pchat_dcc_max_send_cps;
	int pchat_dcc_permissions;
	int pchat_dcc_port_first;
	int pchat_dcc_port_last;
	int pchat_dcc_stall_timeout;
	int pchat_dcc_timeout;
	int pchat_flood_ctcp_num;				/* flood */
	int pchat_flood_ctcp_time;			/* seconds of floods */
	int pchat_flood_msg_num;				/* same deal */
	int pchat_flood_msg_time;
	int pchat_gui_chanlist_maxusers;
	int pchat_gui_chanlist_minusers;
	int pchat_gui_dialog_height;
	int pchat_gui_dialog_left;
	int pchat_gui_dialog_top;
	int pchat_gui_dialog_width;
	int pchat_gui_lagometer;
	int pchat_gui_lang;
	int pchat_gui_pane_divider_position;
	int pchat_gui_pane_left_size;
	int pchat_gui_pane_right_size;
	int pchat_gui_pane_right_size_min;
	int pchat_gui_search_pos;
	int pchat_gui_slist_select;
	int pchat_gui_tab_layout;
	int pchat_gui_tab_middleclose;
	int pchat_gui_tab_newtofront;
	int pchat_gui_tab_pos;
	int pchat_gui_tab_small;
	int pchat_gui_tab_trunc;
	int pchat_gui_transparency;
	int pchat_gui_throttlemeter;
	int pchat_gui_ulist_pos;
	int pchat_gui_ulist_sort;
	int pchat_gui_url_mod;
	int pchat_gui_win_height;
	int pchat_gui_win_fullscreen;
	int pchat_gui_win_left;
	int pchat_gui_win_state;
	int pchat_gui_win_top;
	int pchat_gui_win_width;
	int pchat_identd_port;
	int pchat_irc_ban_type;
	int pchat_irc_join_delay;
	int pchat_irc_notice_pos;
	int pchat_net_ping_timeout;
	int pchat_net_proxy_port;
	int pchat_net_proxy_type;				/* 0=disabled, 1=wingate 2=socks4, 3=socks5, 4=http */
	int pchat_net_proxy_use;				/* 0=all 1=IRC_ONLY 2=DCC_ONLY */
	int pchat_net_reconnect_delay;
	int pchat_notify_timeout;
	int pchat_text_max_indent;
	int pchat_text_max_lines;
	int pchat_url_grabber_limit;

	/* STRINGS */
	char pchat_away_reason[256];
	char pchat_completion_suffix[4];		/* Only ever holds a one-character string. */
	char pchat_dcc_completed_dir[PATHLEN + 1];
	char pchat_dcc_dir[PATHLEN + 1];
	char pchat_dcc_ip[DOMAINLEN + 1];
	char pchat_gui_ulist_doubleclick[256];
	char pchat_input_command_char[4];
	char pchat_irc_extra_hilight[300];
	char pchat_irc_id_ntext[64];
	char pchat_irc_id_ytext[64];
	char pchat_irc_logmask[256];
	char pchat_irc_nick1[NICKLEN];
	char pchat_irc_nick2[NICKLEN];
	char pchat_irc_nick3[NICKLEN];
	char pchat_irc_nick_hilight[300];
	char pchat_irc_no_hilight[300];
	char pchat_irc_part_reason[256];
	char pchat_irc_quit_reason[256];
	char pchat_irc_real_name[127];
	char pchat_irc_user_name[127];
	char pchat_net_bind_host[127];
	char pchat_net_proxy_host[64];
	char pchat_net_proxy_pass[256];
	char pchat_net_proxy_user[256];
	char pchat_stamp_log_format[64];
	char pchat_stamp_text_format[64];
	char pchat_text_background[PATHLEN + 1];
	char pchat_text_font[4 * FONTNAMELEN + 1];
	char pchat_text_font_main[FONTNAMELEN + 1];
	char pchat_text_font_alternative[3 * FONTNAMELEN + 1];
	char pchat_text_spell_langs[64];
