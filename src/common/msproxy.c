...
traverse_msproxy (int sok, char *serverAddr, int port, struct msproxy_state_t *state, netstore *ns_proxy, int csok4, int csok6, int *csok, char bound)
{
	...
	if (!prefs.pchat_net_proxy_auth || !prefs.pchat_net_proxy_user[0] || !prefs.pchat_net_proxy_pass[0] )
		return 1;
	...
	strcpy (data, prefs.pchat_net_proxy_user);		/* Append a username				*/
	data += strlen (prefs.pchat_net_proxy_user)+2;		/* +2 automatically creates second empty string	*/
	...
	*/
	...
	if (flags) {
		...
		ntlm_smb_nt_encrypt(prefs.pchat_net_proxy_pass, challenge, data);		/* Append an NTLM response			*/
		data += 24;	
	} else {
		req.packet.auth2.lm_resp.len	= 24;				/* Fill in LM response security buffer		*/
		req.packet.auth2.lm_resp.alloc	= 24;
		req.packet.auth2.lm_resp.offset	= data - req.packet.auth2.NTLMSSP;
		ntlm_smb_encrypt(prefs.pchat_net_proxy_pass, challenge, data);		/* Append an LM response			*/
		data += 24;
	...
	req.packet.auth2.username_buf.len = strlen(prefs.pchat_net_proxy_user);		/* Username					*/
	...
	strcpy(data, prefs.pchat_net_proxy_user);
